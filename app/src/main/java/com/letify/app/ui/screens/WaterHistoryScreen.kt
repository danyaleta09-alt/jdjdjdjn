...
fun WaterHistoryScreen(onBack: () -> Unit) {
    ...
    // CRITICAL: fillMaxSize + opaque bg so the transparent system status bar
    // paints over our page background (same edge-to-edge contract as every
    // other overlay). The previous fillMaxWidth-only Box left the status-bar
    // region undrawn → opaque/default system bar appeared on this screen only.
